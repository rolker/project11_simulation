...
def _polygon_to_polyline_model(
    name, geometry, center_lat, center_lon, height,
    ...
    return (
        ...
        f'      <pose>{cx:.2f} {cy:.2f} 0 0 0 0</pose>\n'
        f'      <link name="link">\n'
...
def generate_feature_models(features, center_lat, center_lon):
    ...
    for i, pontoon in enumerate(features.pontoons):
        model = _polygon_to_polyline_model(
            ...
            center_lat, center_lon, _PONTOON_HEIGHT,
            ambient='0.4 0.35 0.3 1.0', diffuse='0.5 0.45 0.4 1.0',
        )
        if model:
            models.append(model)
    ...
    for i, bridge in enumerate(features.bridges):
        height = (bridge.clearance if bridge.clearance > 0
                  else _BRIDGE_DEFAULT_HEIGHT)
        model = _polygon_to_polyline_model(
            f'bridge_{i:04d}', bridge.geometry,
            center_lat, center_lon, height,
            ambient='0.5 0.5 0.5 1.0', diffuse='0.6 0.6 0.6 1.0',
        )
        if model:
            models.append(model)
    ...
    for i, beacon in enumerate(features.beacons):
        ...
        models.append(_beacon_model(f'beacon_{i:04d}', x, y))
    ...
    for i, light in enumerate(features.lights):
        ...
        models.append(_light_model(f'light_{i:04d}', x, y))
