...
__doc__ = """
Implement Vincenty's formula to calculate the forward \
(position1+azimuth and range to position2)
and inverse (position1, position2 to azimuth and range) solutions of \
geodesics on the WGS84 ellipsoid.

Vincenty, T. (April 1975a)."DIRECT AND INVERSE SOLUTIONS OF GEODESICS \
0N THE ELLIPSOID WITH APPLICATION
OF NESTED EQUATIONS"
...
def direct(lon1, lat1, azimuth, distance):
    """Calculate position P2 from azimuth and distance from P1 on WGS84.
    ...
    a = 6378137.0        # length of major axis of the ellipsoid
    b = 6356752.314245   # length of minor axis of the ellipsoid
    ...
    phi2 = math.atan2(
        sinU1 * math.cos(sigma) + cosU1 * math.sin(sigma) * cosAlpha1,
        (1 - f) * math.sqrt(
            sinAlpha * sinAlpha
            + pow(sinU1 * math.sin(sigma)
                  - cosU1 * math.cos(sigma) * cosAlpha1, 2)))
    lam = math.atan2(
        math.sin(sigma) * sinAlpha1,
...
def inverse(lon1, lat1, lon2, lat2):
    """Calculate azimuth and distance from P1 to P2 on WGS84.

    @param lon1,lat1: Position P1 in rads
    @param lon2,lat2: Position P2 in rads
    @return: azimuth in rads, distance in meters
    """
    # short circuit if same points, avoids div by zero later
    ...
    while True:
        ...
        if last_lam is not None and abs(last_lam - lam) <= epsilon:
            break
        last_lam = lam

        C = (f / 16.0) * cos2Alpha * (4.0 + f * (4.0 - 3.0 * cos2Alpha))
        lam = L + (1.0 - C) * f * sinAlpha * (
            sigma + C * sinSigma * (
